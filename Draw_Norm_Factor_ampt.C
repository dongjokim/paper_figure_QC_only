...
bool Draw_new_ekrt[kNSC] = { 0, 0, 0, 0, 0 };
...
void Draw_Norm_Factor_ampt(){
				...
				int isc=0;
								...
								upPad->cd();
								TLegend *leg = new TLegend( 0.12, 0.7, 0.7, 0.9, "#LTv_{3}^{2}#GT#LTv_{2}^{2}#GT", "brNDC");
								leg->SetBorderSize(0);
								leg->SetFillStyle(0);
...
//								}	
				...
				y_max = 5e-6;
								...
								upPad2->cd();
								TLegend *leg2 = new TLegend( 0.12, 0.7, 0.7, 0.9, "#LTv_{4}^{2}#GT#LTv_{2}^{2}#GT", "brNDC");
...
//								}	
				...
				y_max = 0.8e-6;
								...
								upPad3->cd();
								TLegend *leg3 = new TLegend( 0.12, 0.7, 0.7, 0.9, "#LTv_{5}^{2}#GT#LTv_{2}^{2}#GT", "brNDC");
...
//									CalculateRatio(gr_C_factor_EKRT[isc][ipar], gr_C_factor[isc], 0)->Draw("same pl");
				...
				y_max = 0.08e-6;
								...
								upPad4->cd();
								TLegend *leg4 = new TLegend( 0.12, 0.7, 0.7, 0.9, "#LTv_{5}^{2}#GT#LTv_{3}^{2}#GT", "brNDC");
...
//									CalculateRatio(gr_C_factor_EKRT[isc][ipar], gr_C_factor[isc], 0)->Draw("same pl");
	...
	if(1){
				int isc=4;
				y_max = 0.5e-6;
								TCanvas *c5 = new TCanvas("c5","c4", 600, 700);
								c5->Draw();
								...
								upPad5->cd();
								TLegend *leg5 = new TLegend( 0.12, 0.7, 0.7, 0.9, "#LTv_{4}^{2}#GT#LTv_{3}^{2}#GT", "brNDC");
...
void LoadSCResults(){
				...
				TFile *fin_data = new TFile("results_0720/results_LHC10h_JFFlucAODLHC10h_Default.root", "read" );
					...
					for(int isc=0; isc<kNSC; isc++){
								...
								/* 
					            TGraphErrors *gr_sc =(TGraphErrors*)fin_data->Get(Form("gr_SC_with_QC_%s", strSCName[isc].Data() ));
                				TGraphErrors *gr_nsc =(TGraphErrors*)fin_data->Get(Form("gr_SC_norm_with_QC_%s", strSCName[isc].Data() ));

			                    gr_C_factor[isc] = Cal_C_factor(gr_nsc, gr_sc);
								//test done
								*/
...
void Load_AMPT(){
	...
	int ms[3] ={kOpenCircle, kOpenSquare, kOpenDiamond};
    ...
    for(int iampt=0; iampt<kNAMPT; iampt++){
            ...
            for( int isc=0; isc<kNSC; isc++){
                ...
                TGraphErrors *gr_nsc =(TGraphErrors*)fin_ampt[iampt]->Get(Form("gr_SC_norm_with_QC_%s", strSCName[isc].Data() ));
                    ...
                    gr_C_factor_AMPT[isc][iampt]->SetMarkerStyle(ms[iampt]);
                    gr_C_factor_AMPT[isc][iampt]->SetMarkerColor(ampt_color[iampt]);
					gr_C_factor_AMPT[isc][iampt]->SetLineStyle(2+iampt);
					gr_C_factor_AMPT[isc][iampt]->SetLineWidth(2);
...
TGraphErrors *CalculateRatio( TGraphErrors* gr1, TGraphErrors* gr2, double xshift){
        ...
        for(int i=0; i<NPoint; i++){
                ...
                gr_ratio->SetPoint( i, x+xshift, ratio-1);
