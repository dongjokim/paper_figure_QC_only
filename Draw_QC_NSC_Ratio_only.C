...
double y_min = -1.5;
double y_max = 1.5;
double y_min_ratio = -1.2;
double y_max_ratio = 1.2;
...
TString datainfo[NSet][3] = {
	"results_0720/results_LHC10h_JFFlucAODLHC10h_Default.root", Form("gr_SC_norm_with_QC_%s", strSCType[isc].Data()), "Data",

	"results_0720/results_AMPT13f3b_JFFluc_KineOnly_Eta0408_Pt0250.root", Form("gr_SC_norm_with_QC_%s", strSCType[isc].Data()), "AMPT Default", 
	"results_0720/results_AMPT13f3c_JFFluc_KineOnly_Eta0408_Pt0250.root", Form("gr_SC_norm_with_QC_%s", strSCType[isc].Data()), "AMPT ST On", 
	"results_0720/results_AMPT13f3a_JFFluc_KineOnly_Eta0408_Pt0250.root", Form("gr_SC_norm_with_QC_%s", strSCType[isc].Data()), "AMPT ST On/ RC Off", 

	"VISH_simdata/AMPT/resultSCR.root", Form("%s_%d", strSCShortName[isc].Data(), 0), "VISH2+1 AMPT eta/s=0.08", 
	"VISH_simdata/AMPT/resultSCR.root", Form("%s_%d", strSCShortName[isc].Data(), 1), "VISH2+1 AMPT eta/s=0.16", 
	"VISH_simdata/KLN/resultSCR.root", Form("%s_%d", strSCShortName[isc].Data(), 0), "VISH2+1 KLN eta/s=0.08",
	"VISH_simdata/KLN/resultSCR.root", Form("%s_%d", strSCShortName[isc].Data(), 1), "VISH2+1 KLN eta/s=0.2",
	"VISH_simdata/Glb/resultSCR.root", Form("%s_%d", strSCShortName[isc].Data(), 0), "VISH2+1 Glb eta/s=0.08",
	"VISH_simdata/Glb/resultSCR.root", Form("%s_%d", strSCShortName[isc].Data(), 1), "VISH2+1 Glb eta/s=0.2",

	"Harris_results/EKRT_eta_020_pt_NSC.root", Form("gr_harri_NSC_%d_pt_1", isc), "EKRT param0 eta/s=0.20",
	"Harris_results/EKRT_eta_par1_pt_NSC.root", Form("gr_harri_NSC_%d_pt_1", isc), "EKRT param1"
};
...
void Draw_QC_NSC_Ratio_only(){
	...
	upPad->cd();
	TLegend *leg = new  TLegend(0.3, 0.6, 0.75, 0.88, "", "brNDC");
	leg->SetFillStyle(0); 
	leg->SetBorderSize(0);
...
//DJ's ask only ratio in 1 canvas
    ...
    c1000->Draw();
    TH2D *href = new TH2D("href1000", Form("NSC(%d,%d)", m, n), 100, 0, 52, 100, -2, 1.5);
    href->SetStats(0);
	...
	c1000->SaveAs(Form("NSC%d%d_ratio_only.eps",m,n));
