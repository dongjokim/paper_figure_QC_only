...
//				gr_SC[im][ipt][isc]->Print();
	...
	for(int ix=0; ix<nx; ix++){
			for(int iy=0; iy<ny; iy++){
					...
					if(ix==0 && iy==0){
							cout << count << endl;
							leg[count] = new TLegend( 0.08, 0.7, 0.48, 0.83, Form("ALICE Preliminary Pb+Pb #sqrt{S_{NN}}=2.76 TeV"), "brNDC" );
							leg_ampt = new TLegend( 0.00, 0.13, 0.4, 0.36, "", "brNDC" );
	...
	for(int ix=0; ix<nx; ix++){
		for(int iy=0; iy<ny; iy++){
			...
			if( count == 0 ) leg[count]->AddEntry( gr_SC_xpt[kQC][count][isc], "SC(3,2)/#LTv_{3}^{2}#GT#LTv_{2}^{2}#GT |#eta| < 0.8", "p");
	...
	p->GetCanvas()->SaveAs("figs/fig5_QConly_xminpt_nSC32_EKRT.eps");
	...
	for(int ix=0; ix<nx; ix++){
			for(int iy=0; iy<ny; iy++){
					int count = ix + nx* iy;
					if( count == 0){
							leg2[count] = new TLegend( 0.01, 0.62, 0.6, 0.79, Form("ALICE Preliminary Pb+Pb #sqrt{S_{NN}}=2.76 TeV"), "brNDC" );
	...
	for(int ix=0; ix<nx; ix++){
		for(int iy=0; iy<ny; iy++){
			...
			if( count == 0 ) leg2[count]->AddEntry( gr_SC_xpt[kQC][count][isc], "SC(4,2)/#LTv_{4}^{2}#GT#LTv_{2}^{2}#GT |#eta| < 0.8", "p");
	...
	p->GetCanvas()->SaveAs("figs/fig5_QConly_xminpt_nSC42_EKRT.eps");
